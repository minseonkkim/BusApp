...
public class StopListSet {
    public static HashMap<Integer, Parse.StopList> StopLists;
    public static int size;

    public static HashMap<Integer, Parse.StopList> getStopLists(){
        if (StopLists != null) return StopLists;

        JSONObject jsonObject = new Parse.GetApiData("STATION").getData();
        ...
        for(int i=0;i<size;i++){
            Parse.StopList a = new Parse.StopList((JSONObject) jsonArray.get(i));
    ...
    public static void StopListPrint(HashMap<Integer, Parse.StopList> StopLists) {
        if (StopLists != null)
